...
contract ChainLinkPriceFeed is IChainLinkPriceFeed, OraclesAggregatorOwnerMixin {
    ...
    modifier onlyOraclesAggregator() {
        require(
            msg.sender == oraclesAggregator,
            "ChainLinkPriceFeed: Caller is not the OraclesAggregator"
        );
        _;
    }

    constructor(
        address _oraclesAggregator,
        uint256 _expiredTime
    ) OraclesAggregatorOwnerMixin(_oraclesAggregator) {
        require(
    ...
    function getPrice(
        address _tokenA,
        address _tokenB
    ) external view onlyOraclesAggregator returns (uint256) {
        require(
            priceFeeds[_tokenA][_tokenB] != address(0),
            "ChainLinkPriceFeed: Price feed not set for this pair"
        );
    ...
    function setPriceFeed(
        ...
        address _tokenB,
        address _priceFeed
    ) external onlyOraclesAggregatorOwner {
        require(_tokenA != address(0), "ChainLinkPriceFeed: Invalid tokenA");
        //require(_tokenB != address(0), "ChainLinkPriceFeed: Invalid tokenB"); tokenB现在可为空用来表示USD
        require(_priceFeed != address(0), "ChainLinkPriceFeed: Invalid priceFeed");
        require(
            priceFeeds[_tokenA][_tokenB] == address(0),
            "ChainLinkPriceFeed: Price feed already set for this pair"
        );
