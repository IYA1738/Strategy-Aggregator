...
contract OraclesAggregator is IOraclesAggregator{
    ...
    uint256 public constant MAX_PRICE_DIFFERENCE_BPS = 5; // 0.05%

    constructor(address _timelockOwner){
        require(_timelockOwner != address(0), "Invalid address");
    ...
    function getPrice(address _tokenA, address _tokenB) external view override returns(uint256){
        ...
        uint256 chainlinkPrice = IChainLinkPriceFeed(_chainLinkPriceFeed).getPrice(_tokenA, _tokenB);
    ...
    function _checkPairValidity(address _tokenA, address _tokenB) internal view{
        require(_tokenA != address(0) && _tokenB != address(0) && _tokenA != _tokenB, "Invalid token pair");
        bool isChainLinkSupported = IChainLinkPriceFeed(_chainLinkPriceFeed).isPairExist(_tokenA, _tokenB);
    ...
    function _checkPriceMatch(uint256 _chainLinkPrice, uint256 _pythPrice) internal pure{
       uint256 priceDiff = _chainLinkPrice > _pythPrice ? _chainLinkPrice - _pythPrice : _pythPrice - _chainLinkPrice;
       ...
       require(priceDiff <= Math.mulDiv(_chainLinkPrice, MAX_PRICE_DIFFERENCE_BPS, BPS), "Price difference too high");
    }
